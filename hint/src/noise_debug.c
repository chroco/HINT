...
{
  int i, j, k, pos, cnt=0;
  double e, max, error = 0., d, e1, ee1=0;

  double pes=0, pesc=0, pescc=0, pe=0, eee=0, eee_new=0;
...
/*      d = d_vector_sum(rules_l[i]->dist, nclass);
      for (k=0; k<nclass; k++) {
	merror[k] = 1. - (rules_l[i]->dist[k] + prior_class[k] * m_param) 
	  / (double)(d + m_param);
      }
      pe = d * d_vector_min_max(&merror[0], prior_class, nclass, &pos);
      eee += pe;
      if (cnt>1 || ((i<n_rules_train-1) && (!rules_l[i]->mark) && color[rules_l[i+1]->cr] == scolor)){
	d_vector_print(rules_l[i]->dist, nclass);
	printf(" of %d is %lf\n", pos, pe);
	pesc += pe;
      }
      pes += pe; */
...
/*	max = d_vector_max_max(distr, prior_class, nclass, &pos); */
	if (max>0) {
	  ...
	  if (deb_dec>10) {
	    max = d_vector_max_max(distr, prior_class, nclass, &pos);
	    e1 = 1. -(max + prior_class[pos] * m_param)/(double)(d + m_param);
	    ee1 += d * e1;
	  }
...
/*      if (cnt>1) pescc += d * e;
      if (cnt>1) {
	printf("CNT %d  (%lf <> %lf) = %lf\n", cnt, pes, d * e, d*e-pes);
	printf("prior: "); d_vector_print(prior_class, nclass); printf("\n");
	d_vector_print(distr, nclass);
	printf(" of %d is %lf (prior %lf  N %lf)\n\n", pos, e, 
	       prior_class[0], d);
      } */
      d_vector_set(distr, nclass, 0.);
      cnt=0;
...
/*
  printf("test old %lf new %lf\n", eee/ n_items_train, eee_new/ n_items_train);
  printf("%lf <> %lf\n-----------\n", pesc, pescc);
*/

  if (deb_dec>9)
    printf("final %lf <--> %lf\n", error/ n_items_train, ee1/ n_items_train);
...
/*    { double ee=0; 
      int i;
      for (i=0; i<ncolors; i++) {
	printf("CCCCCCC COLOR %d\n", i);
	ee += laplace_color_err(ccolor, i);
      }
      printf("NEW ERR = %10.8lf\n", ee);
    } */
...
double repeat_merge(int fixed)
{
  ...
  while (ncolors>fixed && ncolors>1) {
    ...
    if (deb_dec>-1 && e_delta<0) printf("Delta rejected: %8.5g\n", e_delta);
